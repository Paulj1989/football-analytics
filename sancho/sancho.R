...
ggplot(goals, aes(x = reorder(player, value),
                  y = value, group = type, fill = type)) +
  geom_chicklet(position = "stack", alpha = 0.8) +
  coord_flip() +
  geom_hline(yintercept = 0, color = "grey30", size = 1.5) +
  scale_y_continuous(
    labels = c(0, 0.2, 0.4, 0.6, 0.8, 1, 1.2, 1.4),
    breaks = c(0, 0.2, 0.4, 0.6, 0.8, 1, 1.2, 1.4)) +
  scale_x_discrete(labels = c(
    "Jadon Sancho" = expression(bold("Jadon Sancho")),
    "Erling Haaland" = expression(bold("Erling Haaland")),
    "Raphaël Guerreiro" = expression(bold("Raphaël Guerreiro")),
    parse = TRUE)) +
  labs(
    title = glue::glue("The Most Productive Offensive Players in the",
                       "Bundesliga So Far This Season"),
    subtitle = glue::glue("<span style = 'color:#1E88E5;'>Non-Penalty Goals",
                          "</span> + <span style = 'color:#FF0D57;'>Assists", 
                          "</span> Per 90 (npG+A/90) in the Bundesliga\n", 
...
creation <- sancho %>%
  gather(
    key = "type", value = "value", pass_live, pass_dead,
