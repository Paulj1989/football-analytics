pacman::p_load(tidyverse, ggtext, cowplot)
...
p <- ggplot() +
  ...
  theme(
    ...
    strip.text = element_text(size = 9, family = "Fira Code", hjust = 0),
    plot.title = element_textbox_simple(size = 34, family = "Fira Code", hjust = 0.552, color = "#4B5259", width = unit(5, "in")),
    plot.subtitle = element_text(size = 32, family = "Fira Code", hjust = 0.5, color = "#4B5259", margin = margin(5, 0, 20, 0)),
    plot.caption = element_text(size = 8, family = "Fira Code Light", hjust = 1, color = "grey30", margin = margin(-5, 0, 0, 0)),
...
ggdraw(p) +
  ...
  draw_label(label = paste0(str_wrap(ropes, 50)), 0.3, 0.924, hjust = 1, size = 7.5, color = "grey96", fontfamily = "Fira Code Light") +
  draw_label(label = paste0(str_wrap(ladders, 55)), 0.695, 0.924, hjust = 0, size = 7.5, color = "grey96", fontfamily = "Fira Code Light") +
  # borders
  draw_line(x = c(0.5, 0, 0, 0.5), y = c(1, 1, 0, 0), size = 4, color = "#D8282B") +
  draw_line(x = c(0.5, 1, 1, 0.5), y = c(1, 1, 0, 0), size = 4, color = "#284C88") +
  # annotations
  draw_label(label = paste0("Rope Ladder"), 0.49, 0.68, angle = -90, size = 9, color = "mediumpurple2", fontfamily = "Fira Code") +
  draw_label(label = paste0("Salmon Ladder"), 0.284, 0.21, angle = 90, size = 9, color = "#284C88", fontfamily = "Fira Code") +
  draw_label(label = paste0("Tarzan Rope"), 0.444, 0.673, angle = 90, size = 9, color = "#D8282B", fontfamily = "Fira Code") +
  draw_label(label = paste0("Double Salmon Ladder"), 0.54, 0.58, angle = -90, size = 9, color = "#284C88", fontfamily = "Fira Code") +
  draw_label(label = paste0("Rope Climb"), 0.825, 0.62, angle = -90, size = 9, color = "#D8282B", fontfamily = "Fira Code") +
  draw_label(label = paste0("Bungee Rope Climb"), 0.745, 0.59, angle = -90, size = 9, color = "#D8282B", fontfamily = "Fira Code") +
