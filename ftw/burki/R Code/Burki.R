...
ggplot(total_comp, aes(PsxG_90, GA90)) +
...
total_comp %>%
  ...
  labs(
    ...
    subtitle = glue("Using PSxG +/- as a Measure of Goalkeeper Performance Above/Below Expectations")
  )
...
ggplot(total_comp, aes(x = PsxG_90, y = SoTA90)) +
  ...
  geom_mark_hull(
    aes(
      filter = Surname %in% c("Bürki", "Neuer"),
      label = "Bürki & Neuer",
  ...
  geom_mark_hull(
    aes(
      filter = Surname %in% c("Gulácsi", "Sommer"),
...
ggplot(total_comp, aes(x = PsxG_90, y = SoTA90)) +
  ...
  labs(
    title = "Goalkeeper Shot-Stopping & Activity | Bundesliga 18/19 - 19/20",
    subtitle = glue("Post-Shot Expected Goals +/- (PSxG +/-) &
